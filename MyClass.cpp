...
void User::initNode() {
	...
	while (getline(inputFile, line)) {
		...
		if (usertype == "Root") {
			this->addNode(new Root(username, password));
		}
		else if (usertype == "Vip") {
			this->addNode(new Vip(username, password));
		}
		else {
			this->addNode(new Common(username, password));
		}
	}
	inputFile.close();
	return;
...
void User::setType(string usertype, User* userHead) {
	...
	if (usertype == "Root") {
		this->addNode(new Root(this->getUsername(), this->getPassword()));
	}
	else if (usertype == "Vip") {
		this->addNode(new Vip(this->getUsername(), this->getPassword()));
	}
	else {
		this->addNode(new Common(this->getUsername(), this->getPassword()));
	}
...
int Root::deleteUser(User* userHead, Contacts* contactsHead) {
	...
	while (1) {
		temp = userHead;
		int returnValue = 0;
		cout << "请输入您要删除的用户（管理员删除用户时，不会清空用户的数据）：";
		cin >> content;
		cin.ignore(numeric_limits<streamsize>::max(), '\n');
		...
		while (temp->getNext() != nullptr) {
			if (temp->getNext()->getUsername() == content) {
				temp->getNext()->deleteNode(userHead);
				cout << "用户删除成功！";
				saveNode(userHead);
				if (returnValue == 1) {
					...
					logsContent << "删除了用户[" << content << ']' << endl;
...
int Common::deleteUser(User* userHead, Contacts* contactsHead) {
	...
	while (1) {
		...
		if (input == 1) {
			...
			contactsHead->deleteAll(contactsHead);
			contactsHead->saveNode(contactsHead);
			logsSave("注销了自己的账号");
			//直接调用deleteNode删除当前节点
			currUser->deleteNode(userHead);
			cout << "账户注销成功！" << endl;
			saveNode(userHead);
			Sleep(2000);
			return 1;
...
int Vip::deleteUser(User* userHead, Contacts* contactsHead) {
	...
	int input, flag = 0;
	//对于Vip用户的关照
	cout << "亲爱的Vip用户：您正在进行当前用户：" << currUser->getUsername() << " 的注销操作！" << endl;
	while (1) {
		cout << "是否确认注销？[1]确认  [0]取消：" << endl;
		if (!(cin >> input) || input < 0 || input > 1) {
			cout << "数据输入错误，请重新输入！" << endl;
...
void Contacts::initNode() {
	...
	while (getline(inputFile, line)) {
		...
		this->addNode(new Contacts(telephone, mobilephone, qq, name, unit, address, sort, email));
	}
	inputFile.close();
	return;
}
	...
	while (1) {
		...
		while (temp->getNext() != nullptr) {
			if (temp->getNext()->getName() == content) {
				...
				temp->getNext()->deleteNode(contactsHead);
				stringstream logsContent;
	...
	while (1) {
		...
		while (temp->getNext() != nullptr) {
			if (temp->getNext()->getName() == content) {
				...
				temp->getNext()->telephone = telephone;
				temp->getNext()->mobilephone = mobilephone;
				temp->getNext()->qq = qq;
				temp->getNext()->name = name;
				temp->getNext()->unit = unit;
				temp->getNext()->address = address;
				temp->getNext()->sort = sort;
				temp->getNext()->email = email;
				stringstream logsContent;
...
void Contacts::swapNode(Contacts* x, Contacts* y, Contacts* contactsHead) {
	//找出四个节点
	Contacts* temp = contactsHead;
	Contacts* pre = nullptr;
	Contacts* lastest = y->getNext();
	//循环找出pre
	while (temp->getNext() != nullptr) {
		if (temp->getNext()->getNext() == x) {
			pre = temp->getNext();
			break;
		}
		temp = temp->getNext();
	}
	//如果没找到pre（pre是第一个有用节点），防止调用空指针
	if (pre == nullptr) {
		return;
	}
	//进行节点交换
